...
/*
Using AnnotationsÂ¶
Creating a complete forms solution can often be tedious: youâ€™ll create some domain model object, an input filter for validating it, a form object for providing a representation for it, and potentially a hydrator for mapping the form elements and fieldsets to the domain model. Wouldnâ€™t it be nice to have a central place to define all of these?
...
Filter: provide a specification for a filter to use on a given element. Expects an associative array of values, with a â€œnameâ€ key pointing to a string filter name, and an â€œoptionsâ€ key pointing to an associative array of filter options for the constructor: @Filter({"name": "Boolean", "options": {"casting":true}}). This annotation may be specified multiple times.
...
Options: options to pass to the fieldset or form that are used to inform behavior â€“ things that are not attributes; e.g. labels, CAPTCHA adapters, etc. The annotation expects an associative array: @Options({"label": "Username:"}).
...
Validator: provide a specification for a validator to use on a given element. Expects an associative array of values, with a â€œnameâ€ key pointing to a string validator name, and an â€œoptionsâ€ key pointing to an associative array of validator options for the constructor: @Validator({"name": "StringLength", "options": {"min":3, "max": 25}}). This annotation may be specified multiple times.
...
If youâ€™re not using Composer, visit the Doctrine project website for more details on installation.
