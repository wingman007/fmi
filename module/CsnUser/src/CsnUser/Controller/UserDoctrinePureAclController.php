...
class UserDoctrinePureAclController extends AbstractActionController
{
	private $usersTable;
	// for Doctrine
...
//-		$this->registerDocrineClassLoader();		
	...
	{
/* !!! WE DON'T need to repeat the Authorization code anymore. It is all moved to CsnUser\Module.php
	// -------------------------------------------------
		// I) Authorization
...
//				$this->getUsersTable()->insert($data);
				...
				return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-doctrine-pure-acl', 'action' => 'index'));										
			}
		}		
	...
	{
/* Naive approach both the decision making and the code are in the action
		// ------------------------ The block for Authorization
		$auth = new \Zend\Authentication\AuthenticationService();
		if (!$auth->hasIdentity()) return $this->redirect()->toRoute('auth/default', array('controller' => 'index', 'action' => 'login'));
		$user = $auth->getIdentity();
...
*/	
		...
		if (!$id) return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-doctrine-pure-acl', 'action' => 'index'));
...
*/		
        ...
        try {
			...
			$this->redirect()->toRoute('csn_user/default', array('controller' => 'user-doctrine-pure-acl', 'action' => 'index'));	
        }
