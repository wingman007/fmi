...
class UserRowGatewayFeatureController extends AbstractActionController
{
	private $usersTable;
        ...
        if ($request->isPost()) {
			...
			$form->setData($request->getPost());
			 if ($form->isValid()) {
				...
				return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-row-gateway-feature', 'action' => 'index'));										
			}
		}		
	...
	public function updateAction()
	{
		...
		if (!$id) return $this->redirect()->toRoute('csn_user/default', array('controller' => 'user-row-gateway-feature', 'action' => 'index'));
		$form = new UserForm();
		$request = $this->getRequest();
        if ($request->isPost()) {
			$form->setInputFilter(new UserFilter());
